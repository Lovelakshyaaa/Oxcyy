      ...
      builder: (context, isPlayerExpanded, _) {
        return AnimatedContainer(
          ...
          child: Stack(
            children: [
              MiniPlayerView(
                  mediaItem: _currentMediaItem!, audioHandler: audioHandler),
              if (isPlayerExpanded)
                AnimatedOpacity(
                  duration: const Duration(milliseconds: 400),
                  opacity: isPlayerExpanded ? 1.0 : 0.0,
...
class MiniPlayerView extends StatelessWidget {
  ...
  const MiniPlayerView(
      {Key? key, required this.mediaItem, required this.audioHandler})
      : super(key: key);
                    ...
                    child: FadeInAnimation(
                      child: Column(
                        children: [
                          ...
                          _SongInfoCard(
                              mediaItem: mediaItem, audioHandler: audioHandler),
...
class _Artwork extends StatelessWidget {
  ...
  const _Artwork({Key? key, required this.mediaItem, required this.size})
      : super(key: key);
  ...
  Widget _defaultArtwork({double? size}) => Container(
      width: size,
      height: size,
...
class _HighResArtwork extends StatefulWidget {
  ...
  const _HighResArtwork(
      {Key? key, required this.mediaItem, this.size, this.isBackground = false})
      : super(key: key);
...
class __HighResArtworkState extends State<_HighResArtwork> {
  ...
  Future<void> _fetchArtwork() async {
    ...
    dynamic albumId = widget.mediaItem.extras?['albumId'];
  ...
  Widget build(BuildContext context) {
    ...
    } else if (widget.mediaItem.artUri != null) {
      return CachedNetworkImage(
        ...
        useOldImageOnUrlChange: true,
        fadeInDuration: const Duration(milliseconds: 300),
        placeholder: (context, url) => _defaultArtwork(size: widget.size),
        errorWidget: (_, __, ___) => _defaultArtwork(size: widget.size),
      );
    }
          ...
          children: [
            IconButton(
              icon: Icon(Icons.shuffle,
                  color: isShuffleEnabled ? Colors.purpleAccent : Colors.white),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                        color: Colors.purple.withOpacity(0.4), blurRadius: 15)
                  ]),
              child: IconButton(
                iconSize: 50,
