...
class PlayerScreen extends StatelessWidget {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(song.title, style: Theme.of(context).textTheme.headline5, textAlign: TextAlign.center),
          const SizedBox(height: 10),
          Text(song.artist, style: Theme.of(context).textTheme.subtitle1, textAlign: TextAlign.center),
          const Spacer(),
  ...
  Widget _buildArtwork(Song song, {required bool isExpanded}) {
    return song.thumbUrl.isNotEmpty
        ? Image.network(song.thumbUrl, height: isExpanded ? 250 : 50)
        : FutureBuilder<Uint8List?>(
            future: Provider.of<MusicProvider>(_scaffoldKey.currentContext!).getArtwork(int.parse(song.id), ArtworkType.AUDIO),
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                return Image.memory(snapshot.data!, height: isExpanded ? 250 : 50);
              } else {
                return Container(height: isExpanded ? 250 : 50, color: Colors.grey, child: const Icon(Icons.music_note));
              }
            },
          );
  }
  ...
  Widget _buildControls(BuildContext context, MusicProvider musicProvider) {
    return Row(
      ...
      children: [
        IconButton(icon: const Icon(Icons.shuffle), onPressed: () => musicProvider.toggleShuffle()),
        IconButton(icon: const Icon(Icons.skip_previous), onPressed: () {}), // Add logic
        IconButton(
        ...
        IconButton(icon: const Icon(Icons.skip_next), onPressed: () {}), // Add logic
        IconButton(icon: const Icon(Icons.repeat), onPressed: () => musicProvider.cycleRepeatMode()),
      ],
    );
  }
